...
from ..application.context.agent_input_builder import AgentInputBuilder
...
from ..channels.feishu.card_handler import FeishuCardHandler
from ..channels.feishu.command_router import FeishuCommandRouter
from ..channels.feishu.renderer import FeishuRenderer
...
from ..executors.document_executor import DocumentAppendExecutor, DocumentExecutor
...
from ..modules.oauth.resume_service import OAuthResumeService
...
def _bind_card_message(store):
    """Presenter 发送确认卡片后把消息 ID 绑定到 Action（可靠性机制）。"""

    async def bind(action_id: str, message_id: str) -> None:
        await store.set_pending_action_card_message(action_id, message_id)
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        dispatcher.register("task.create", task_service.prepare_create)
        dispatcher.register("task.query", task_service.query)
        ...
        presenter = FeishuResponsePresenter(
            ...
            renderer,
            transport,
            card_message_binder=_bind_card_message(store),
        )
        plans = PlanModule(store, timezone=config.timezone)
        command_router = FeishuCommandRouter(plans, app_id=config.feishu_app_id)
        workflow.presenter = presenter
        workflow.command_router = command_router
        ...
        app.state.openai_adapter = OpenAIAdapter(config, store, dispatcher)
        ...
        oauth.on_authorized = oauth_resume.resume
        ...
        card_callbacks = FeishuCardHandler(
            pending_action_service,
